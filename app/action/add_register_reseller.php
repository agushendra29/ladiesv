...
try {
    ...
    if (!is_array($payload)) {
        echo json_encode(['ok' => false, 'message' => 'Payload JSON tidak valid.']);
        exit;
    }
    ...
    if (!empty($birth_input)) {
        if (preg_match('/^(0[1-9]|[12][0-9]|3[01])-(0[1-9]|1[0-2])-(\d{4})$/', $birth_input, $m)) {
            ...
            echo json_encode(['ok' => false, 'message' => 'Format tanggal salah! Gunakan dd-mm-yyyy.']);
            exit;
        }
    }
    ...
    if (!empty($sup_npwp) && !preg_match('/^[0-9]{15,16}$/', $sup_npwp)) {
        echo json_encode(['ok' => false, 'message' => 'NPWP harus 15 atau 16 digit angka.']);
        exit;
    }

    if ($referal_code === '') {
        echo json_encode(['ok'=>false,'message'=>'Kode referal wajib diisi']);
        exit;
    }

    if (!$sup_name || !$sup_rekening || !$sup_bank || !$birth_date || !$sup_akun ||
        !$sup_contact || !$sup_email || !$sup_address) {
        echo json_encode(['ok' => false, 'message' => 'Field * wajib diisi.']);
        exit;
    }
    ...
    if ($referal_code !== '') {
        ...
        if (!$ref) {
            echo json_encode(['ok' => false, 'message' => 'Kode referal tidak ditemukan.']);
            exit;
        }
    ...
    if (!$suppliar_id) {
        echo json_encode(['ok' => false, 'message' => 'Gagal menambahkan suppliar.']);
        exit;
    }
    ...
    if (empty($items)) {
        echo json_encode(['ok' => false, 'message' => 'Produk wajib diisi.']);
        exit;
    }
    ...
    foreach ($items as $p) {
        ...
        if (!$product) {
            $pdo->rollBack();
            echo json_encode(['ok' => false, 'message' => "Produk ID {$pid} tidak ditemukan."]);
            exit;
        }
        ...
        if (!$fromStock || (int)$fromStock['stock'] < $qty) {
            $pdo->rollBack();
            echo json_encode(['ok' => false, 'message' => "Stok {$product->product_name} kurang."]);
            exit;
        }
