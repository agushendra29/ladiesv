...
function getTotalQty(PDO $pdo, string $startDate, string $endDate, int $userId): int
{
    ...
    /**
     * Aturan Poin:
     * 1️⃣ type = 'penjualan' dan is_refund = 0
     * 2️⃣ Jika menjual ke customer_role_id = 5 → tidak dapat poin
     * 3️⃣ Jika suppliar punya parent (suppliar_parent_id = :uid) → parent dapat poin
     */
    ...
    $sql = "
        ...
        WHERE th.type = 'penjualan'
          ...
          AND (
              -- poin dari penjualan langsung user (selain ke reseller)
              (th.suppliar_id = :uid AND th.customer_role_id != 5)

              OR

              -- poin dari penjualan anak (supplier yang parent-nya adalah user ini)
              (th.suppliar_parent_id = :uid)
          )
    ";
