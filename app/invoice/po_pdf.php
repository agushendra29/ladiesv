...
$stmt = $pdo->prepare("
    SELECT th.invoice_number, th.type, th.created_at, th.payment_type, th.bank_type,
           ...
           s2.id AS customer_id, s2.name AS customer_name, s2.role_id AS customer_role, s2.suppliar_code AS customer_code,
           s2.parent_id, s2.address AS customer_address,
...
$pdf->Rect($borderMargin, $borderMargin, 210 - 2 * $borderMargin, 148 - 2 * $borderMargin);
...
$pengirim = ($header['role_id']==1||$header['role_id']==10)
    ? 'Head Office'
    : $header['suppliar_name'];

/* --- Penerima --- */
if(!$header['customer_id'] || $header['customer_id'] == 0){
    $penerima = "Penjualan Pribadi";
    $idPengundang = '';
}else{
    if($header['customer_role'] == 5 && $header['parent_id']){
        // Ambil data parent
        $stmtParent = $pdo->prepare("SELECT name, suppliar_code FROM suppliar WHERE id=:id LIMIT 1");
        $stmtParent->execute([':id'=>$header['parent_id']]);
        $parent = $stmtParent->fetch(PDO::FETCH_ASSOC);
        $penerima = formatMember($header['customer_name'],$header['customer_role'],$header['customer_code']);
        $idPengundang = $parent ? formatMember($parent['name'], $header['customer_role'], $parent['suppliar_code']) : '';
    }else{
        $penerima = formatMember($header['customer_name'],$header['customer_role'],$header['customer_code']);
        $idPengundang = '';
    }
...
if($idPengundang){
    $pdf->Cell($labelW,$rowH,"ID Pengundang:",0,0,'L');
    $pdf->Cell($valueW,$rowH,$idPengundang,0,1,'L');
}
...
$alamatWidth = $contentWidth * 0.4;
$pdf->MultiCell($alamatWidth,5,preg_replace('/,\s*,/', ',', $alamatLengkap));
